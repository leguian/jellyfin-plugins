...
| Fonction | Détail |
| --- | --- |
| Ordre | Glisser-déposer des sections (souris et tactile), ou au clavier / lecteur d'écran / télécommande depuis la poignée |
...
- Sans pointeur : la poignée est un bouton. Flèches haut / bas dessus = la ligne monte ou descend d'un cran (une section sort de son dossier à une extrémité et entre dans le dossier qu'elle rencontre ; un dossier se déplace avec son contenu). Entrée, appui tactile ou activation par un lecteur d'écran = menu **Monter** / **Descendre**, qui reste ouvert ; les déplacements impossibles y sont grisés. Le focus suit la ligne déplacée et sa nouvelle position est annoncée (région `role=status`). Rien ne bouge pendant une recherche.
- Accessibilité de la fenêtre : dialogue nommé par son titre, Tab / Maj+Tab restent dans la fenêtre (ou dans le menu ouvert par-dessus), menus `role=menu` pilotables aux flèches, Début et Fin, focus rendu au bouton qui a ouvert le menu puis, à la fermeture, à l'élément qui a ouvert l'éditeur (sans faire défiler la page ; aucun focus rendu quand l'éditeur a été ouvert depuis le menu latéral). Intégré à la page d'administration, l'éditeur est un `role=group` non modal.
...
- **Enregistrer** applique la disposition. **Annuler**, le bouton de fermeture, Échap ou un clic hors de la fenêtre ferment l'éditeur sans enregistrer ; s'il y a des modifications, une question remplace d'abord le pied de la fenêtre (**Continuer la modification** par défaut, Échap = continuer). Même question avant **Réinitialiser**, et pour **Annuler** dans l'éditeur de la page d'administration. Plus aucun `window.confirm` dans le script client.
...
- Rotation en pause au survol, au focus clavier, onglet masqué ; désactivée si le système demande de réduire les animations. Bouton pause / lecture à côté des points (présent seulement quand le hero tourne tout seul) : l'état est conservé tant que la session dure. Flèches, points, touches gauche / droite (hors mode TV), balayage tactile. Chaque point a une zone de clic d'au moins 24 px (WCAG 2.5.8) : les points gardent leur taille, mais l'écart visible entre eux est celui qu'imposent ces zones, posées côte à côte.
- Affichage : pleine largeur, départ sous le menu du haut, fondu dans la page en bas (masque CSS, donc quel que soit le fond du thème). Tant que le menu survole le hero, il devient transparent avec un léger dégradé sombre (classe `ch-hero-under-header` sur `<html>`) ; il retrouve son fond une fois le hero dépassé, en quittant l'accueil ou en passant sur l'onglet Favoris (observation de la classe `is-active` du panneau d'onglet). La transition du menu n'est posée que tant qu'un hero est affiché (classe `ch-hero-shown` sur `<html>`). Les marges négatives sont mesurées par le script (hauteur du menu, marges du thème), pas codées en dur.
...
Les trois actions destructrices de la page (vider la disposition par défaut, réinitialiser un utilisateur, supprimer une miniature) posent leur question dans la page et non par `window.confirm` : `role=alertdialog`, focus sur **Cancel**, Échap annule, Tab reste entre les deux réponses, et la question nomme l'utilisateur ou la forme concernée. Plus aucun `window.confirm` dans le plugin.
...
| GET/POST | `/CustomizedHome/DefaultLayout` | admin | disposition par défaut ; 503 si le fichier de configuration ne peut pas être écrit, la disposition précédente reste alors en place |
...
| GET | `/CustomizedHome/GenreImages/Image?name=&shape=&v=` | anonyme | miniature d'un genre pour une forme (comme toute image Jellyfin) ; 404 s'il n'y en a pas, 503 si le disque la refuse (l'URL est servie comme immuable : un 404 serait mis en cache bien après le retour à la normale) |
...
| `specs/a11y.spec.ts` | éditeur sans pointeur : déplacement au clavier, menu de déplacement, focus, questions avant d'abandonner des modifications |
| `specs/hardening.spec.ts`, `specs/keys.spec.ts`, `specs/compat.spec.ts` | clés hostiles (`__proto__`, `constructor`), clés tirées de titres non latins, replis CSS des vieux moteurs |
...
2. Jellyfin ne supprime que le dossier du plugin. Restent dans `<config>/plugins/configurations/` : `Jellyfin.Plugin.CustomizedHome.xml` (options et disposition par défaut) et le dossier `Jellyfin.Plugin.CustomizedHome/` (`users/*.json` : une disposition par utilisateur ; `genres/` : miniatures et index). Les supprimer à la main pour tout effacer ; les garder si une réinstallation est prévue, les dispositions sont alors reprises. C'est voulu : désinstaller puis réinstaller est un geste de dépannage courant, il ne doit pas effacer la disposition de chaque utilisateur (le chemin conservé est écrit dans le journal du serveur à la désinstallation). En revanche, la disposition d'un utilisateur supprimé de Jellyfin est supprimée avec lui (événement du serveur), et au démarrage du serveur le plugin supprime les fichiers de disposition dont l'utilisateur n'existe plus : cela couvre les suppressions faites pendant que le plugin était arrêté ou désinstallé. Un fichier dont le nom n'est pas un identifiant d'utilisateur n'est jamais touché.
...
- Miniatures de genres : type détecté sur les octets (le type déclaré n'est jamais cru), SVG refusé, 5 Mo max, fichier nommé par hash du genre, forme et version (jamais par l'entrée utilisateur), servi avec `nosniff`. Un remplacement écrit un nouveau fichier puis l'index, puis supprime l'ancien : une URL versionnée, mise en cache comme immuable par les clients, ne sert jamais d'autres octets. Index illisible (fichier verrouillé) : rien n'est écrasé, l'API répond 503.
