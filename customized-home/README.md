...
- Confidentialité de la disposition par défaut : elle est écrite par un administrateur qui voit toutes les médiathèques. Avant envoi à un autre utilisateur, le serveur retire les sections « Ajouts récents » des médiathèques auxquelles il n'a pas accès et n'envoie aucun nom de médiathèque ; le client n'enregistre plus ce nom avec la disposition et le reconstruit à partir des médiathèques de l'utilisateur (« médiathèque indisponible » sinon).
- Les envois de disposition sont limités à 2 Mo (`RequestSizeLimit`) : sans limite, le corps était entièrement désérialisé avant que la validation ne le refuse.
